...
from qtvcp.widgets.versa_probe import VersaProbe as VP
...
"""%(  os.path.expanduser('~/linuxcnc'),
    ...
    def class_patch__(self):
       ...
       # class patch VersaProbe function
        self.super_vp_start_probe = VP.start_probe
        VP.start_probe = self.start_probe
        self.super_vp_process_finished = VP.process_finished
        VP.process_finished = self.versa_process_finished

    # class patch versa probe to call MDI before probing
    def start_probe(self, cmd):
        # ACTION.CALL_MDI_WAIT('(msg,Before)')
        ACTION.CALL_MDI_WAIT("M64 P3")
        self.super_vp_start_probe(self.probe,cmd)

    # class patch versa probe to call MDI after probing
    def versa_process_finished(self, exitCode, exitStatus):
        self.super_vp_process_finished(self.probe, exitCode, exitStatus)
        # ACTION.CALL_MDI('(msg,After)')
        ACTION.CALL_MDI("M65 P3")
